...
extern void *alloc_memory(const size_t size);
extern void dump_N_state_set(N_state_set_t *p);

static D_state_t *register_D_state(N_state_set_t *s);
static D_state_t *fetch_unvisited_D_state(void);
static dlist_t *compute_reachable_N_state(D_state_t *dstate);
static void add_N_state(N_state_set_t *state, const int s);
static void mark_empty_transition(N_state_set_t *state, const int s);
static void collect_empty_transition(N_state_set_t *state);
static int equal_N_state_set(const N_state_set_t *a, const N_state_set_t *b);
static void dump_dfa(void);

static int dfa_nstate = 0;
