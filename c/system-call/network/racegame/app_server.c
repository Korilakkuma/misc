...
static void receive_data(void) {
  ...
  for (int i = 0; i < number_of_players; i++) {
    ...
    if (players[offset + DAMAGE] >= MAX_DAMAGE) {
      int grade_offset = (number_of_players - number_of_crashed - 1) * GRADE_SIZE;

      grades[grade_offset + ENTRY_NUMBER] = i;
      strcpy(&grades[grade_offset + ENTRY_NAME], names[i]);
      number_of_crashed++;
    }

    if (players[offset + STAGE] > number_of_statges) {
